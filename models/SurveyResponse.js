...
const { getConfirmationText, appointments } = require("../survey");
...
SurveyResponseSchema.statics.advanceSurvey = function (args, cb) {
  ...
  function processInput() {
    ...
    function reask(prompt) {
      cb.call(surveyResponse, null, surveyResponse, responseLength, prompt);
    }
    ...
    if (currentQuestion.name == "appointment") {
      if (questionResponse.answer > appointments.length) {
        return reask('Your input is incorrect. Again.');
      }
      console.log(`To send sms to ${phone}, ${getConfirmationText(questionResponse.answer - 1)}`)
      sendSms(phone, getConfirmationText(questionResponse.answer - 1));
    }
